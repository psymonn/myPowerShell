 $JENKINS_URL = "http://localhost:8080"
    ...
    $JOB_URL = "http://localhost:8080/job/ProjectDemo2%20Pipeline"
    Invoke-WebRequest $JOB_URL -Method GET  -Headers $headers
    ...
    if ($action -eq "start") {
        ...
        for($t = $tries; $t -gt 0; $t--) {
            ...
            try {
                ...
                ($jobsubmit.BaseResponse).GetResponseStream()
                ($jobsubmit.Headers).GetEnumerator()
                $jobsubmit.StatusDescription.
                $jobsubmit.StatusCode
                ($jobsubmit.Content).split()
                ($jobsubmit.RawContent).split()
                $jobsubmit.RawContentStream
                $jobsubmit.
